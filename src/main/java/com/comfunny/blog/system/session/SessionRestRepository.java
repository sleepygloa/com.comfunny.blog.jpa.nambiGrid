...
public interface SessionRestRepository extends JpaRepository<SessionDto, Long> {
    ...
    @Query(value = "select id, name, email, CASE WHEN role = 'ADMIN' THEN 'A' WHEN role = 'USER' THEN 'U' ELSE 'G' END AS role  from user where email = :userEmail", nativeQuery = true)
